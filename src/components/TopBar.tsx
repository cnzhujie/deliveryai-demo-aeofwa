      ...
      <header className="sticky top-0 z-30 border-b border-charcoal-900/5 bg-rice-50/95 backdrop-blur-xl dark:border-white/5 dark:bg-charcoal-900/95">
        <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 lg:px-6">
          <button onClick={() => onView('menu')} className="flex items-center gap-2 text-left">
            ...
            <span className="hidden sm:block"><strong className="block leading-4 text-charcoal-900 dark:text-rice-50">{t('common.brand_name')}</strong><small className="text-charcoal-500 dark:text-rice-200">{t('common.subtitle')}</small></span>
          </button>
          ...
          <Dialog>
            ...
            <DialogContent title={t('common.member_title')}>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-white p-4 shadow-sm dark:bg-charcoal-800"><p className="text-xs text-charcoal-500 dark:text-rice-200">{t('common.queue')}</p><p className="mt-2 text-2xl font-extrabold text-charcoal-900 dark:text-rice-50">A018</p><p className="text-xs text-chili-500">{t('common.queue_ahead')}</p></div>
                <div className="rounded-2xl bg-white p-4 shadow-sm dark:bg-charcoal-800"><p className="text-xs text-charcoal-500 dark:text-rice-200">{t('common.benefits')}</p><p className="mt-2 text-2xl font-extrabold text-charcoal-900 dark:text-rice-50">4 <small className="text-sm">{t('common.tickets')}</small></p><p className="text-xs text-amber-500">{t('common.coupon')}</p></div>
              </div>
            </DialogContent>
          </Dialog>
          <Button variant="outline" size="icon" onClick={onConsole} aria-label={t('common.aria_console')}><LayoutDashboard size={18} /></Button>
          <Button variant="outline" size="icon" onClick={onToggleDark} aria-label={dark ? t('common.aria_dark') : t('common.aria_light')}>
