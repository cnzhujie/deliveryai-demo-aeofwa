    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent title={t('console.title')} className="md:max-w-2xl">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <section className="rounded-2xl bg-white p-4 shadow-sm dark:bg-charcoal-800">
            <div className="flex items-center justify-between"><h3 className="font-bold text-charcoal-900 dark:text-rice-50">{t('console.table_fulfillment')}</h3><span className="rounded-full bg-rice-100 px-3 py-1 text-xs font-bold text-charcoal-500 dark:bg-charcoal-700 dark:text-rice-200">{tableLabel}</span></div>
            <div className="mt-4 grid grid-cols-2 gap-2">{stageKeys.map((value) => { const Icon = stageIcons[value]; return <button key={value} onClick={() => onStage(value)} className={`flex items-center gap-2 rounded-xl border p-3 text-left text-sm font-bold transition ${stage === value ? 'border-chili-500 bg-chili-50 text-chili-600 dark:border-chili-500 dark:bg-chili-500/15 dark:text-chili-100' : 'border-charcoal-900/5 bg-rice-50 text-charcoal-500 dark:border-white/5 dark:bg-charcoal-700 dark:text-rice-200'}`}><Icon size={16} />{t(`console.stage.${value}`)}</button> })}</div>
          </section>
          <section className="rounded-2xl bg-white p-4 shadow-sm dark:bg-charcoal-800">
            <div className="flex items-center justify-between"><h3 className="font-bold text-charcoal-900 dark:text-rice-50">{t('console.service_response')}</h3><span className={`rounded-full px-3 py-1 text-xs font-bold ${waiting ? 'bg-amber-100 text-amber-500 dark:bg-amber-500/20 dark:text-amber-100' : 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/15 dark:text-emerald-400'}`}>{t('console.waiting_count', { count: waiting })}</span></div>
        ...
        <section className="mt-4 rounded-2xl bg-white p-4 shadow-sm dark:bg-charcoal-800">
          <div className="flex items-center justify-between"><h3 className="font-bold text-charcoal-900 dark:text-rice-50">{t('console.soldout_title')}</h3><span className="flex items-center gap-1 text-xs text-charcoal-500 dark:text-rice-200"><ToggleLeft size={16} />{t('console.soldout_hint')}</span></div>
          <div className="scrollbar-none mt-4 flex gap-2 overflow-x-auto pb-1">{products.map((product) => { const unavailable = soldOut.includes(product.id); return <button key={product.id} onClick={() => onSoldOut(product.id)} className={`flex shrink-0 items-center gap-2 rounded-xl border px-3 py-2 text-sm font-semibold ${unavailable ? 'border-chili-500/30 bg-chili-50 text-chili-600 dark:border-chili-500/30 dark:bg-chili-500/15 dark:text-chili-100' : 'border-charcoal-900/5 bg-rice-50 text-charcoal-500 dark:border-white/5 dark:bg-charcoal-700 dark:text-rice-200'}`}>{unavailable ? <XCircle size={15} /> : <CheckCircle2 size={15} />}{t(product.name)}</button> })}</div>
        </section>
