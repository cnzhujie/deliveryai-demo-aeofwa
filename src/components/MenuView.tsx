...
export function MenuView({ diners, soldOut, onAdd }: MenuViewProps) {
  ...
  return (
    <section className="min-w-0">
      <div className="relative overflow-hidden rounded-3xl bg-charcoal-900 p-5 text-white shadow-card sm:p-7 dark:bg-black dark:ring-1 dark:ring-white/10">
      ...
      <div className="mt-3 grid gap-4 sm:grid-cols-2">
        {visible.map((product, index) => {
          ...
          return (
            <article key={product.id} className="group animate-rise overflow-hidden rounded-3xl border border-charcoal-900/5 bg-white shadow-card transition hover:-translate-y-1 dark:border-white/5 dark:bg-charcoal-800" style={{ animationDelay: `${index * 40}ms` }}>
              <div className="relative h-40 overflow-hidden">
                ...
                {unavailable && <span className="absolute inset-0 flex items-center justify-center bg-white/55 text-lg font-extrabold text-charcoal-900 backdrop-blur-sm dark:bg-charcoal-900/60 dark:text-rice-50">{t('menu.sold_out')}</span>}
      ...
      <Dialog open={!!selected} onOpenChange={(open) => { if (!open) { setSelected(null); setShowRiskWarning(false) } }}>
        <DialogContent title={selected ? t(selected.name) : ''}>
          {selected && <>
            ...
            <div className="mt-5"><p className="mb-3 flex items-center gap-2 text-sm font-bold text-charcoal-900 dark:text-rice-50"><Users size={16} />{t('menu.who_ordered')}</p><div className="flex gap-2">{diners.map((name) => <button key={name} onClick={() => setDiner(name)} className={`rounded-full px-4 py-2 text-sm font-bold ${diner === name ? 'bg-amber-400 text-charcoal-900' : 'bg-white text-charcoal-500 dark:bg-charcoal-800 dark:text-rice-200'}`}>{name}</button>)}</div></div>
