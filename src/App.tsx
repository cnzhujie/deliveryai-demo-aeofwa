...
export default function App() {
  ...
  const handleToggleDark = () => {
    toggleDark()
    dispatch({ type: 'SET_MESSAGE', message: dark ? t('common.toast_light') : t('common.toast_dark') })
  }
        ...
        <main className="mx-auto grid max-w-7xl gap-6 px-4 py-5 pb-28 lg:grid-cols-3 lg:px-6 lg:py-7 lg:pb-8">
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-28">
              ...
              <div className="mt-4 rounded-2xl border border-amber-400/30 bg-amber-100/70 p-4 text-sm text-charcoal-700 dark:border-amber-400/30 dark:bg-amber-500/15 dark:text-amber-100">
...
function MobileNav({ active, icon: Icon, label, badge, onClick }: { active: boolean; icon: typeof MenuIcon; label: string; badge?: number; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`relative flex flex-col items-center gap-1 rounded-xl py-2 text-xs font-semibold transition ${active ? 'bg-chili-50 text-chili-500 dark:bg-chili-500/15' : 'text-charcoal-500 dark:text-rice-200'}`}>
